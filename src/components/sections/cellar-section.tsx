...
const STATUS_COLOR: Record<WineStatus, { text: string; bg: string; border: string; dot: string }> = {
  ready: { text: 'var(--color-gold)', bg: 'rgba(201,168,76,0.14)',  border: 'rgba(201,168,76,0.45)',  dot: 'var(--color-gold)' },
  hold:  { text: 'var(--color-text-secondary)', bg: 'rgba(212,197,176,0.10)', border: 'rgba(212,197,176,0.30)', dot: 'var(--color-text-secondary)' },
  peak:  { text: '#E06070', bg: 'rgba(196,30,58,0.18)',   border: 'rgba(196,30,58,0.55)',   dot: '#C41E3A' },
  past:  { text: 'var(--color-text-muted)', bg: 'rgba(155,139,122,0.10)', border: 'rgba(155,139,122,0.30)', dot: 'var(--color-text-muted)' },
};
...
function MiniBottle({ type }: { type: WineType }) {
  const fill =
    type === 'red' ? 'var(--color-wine-red)'
    : type === 'white' ? 'var(--color-paper)'
    : 'var(--color-gold)'; // sparkling: gold
  const stroke = type === 'white' ? 'rgba(0,0,0,0.18)' : 'rgba(0,0,0,0.35)';
  return (
    <svg width={36} height={96} viewBox="0 0 36 96" aria-hidden>
      {/* neck */}
      <rect x={15} y={4} width={6} height={20} rx={1.5} fill="var(--color-border)" />
      {/* foil */}
      <rect x={14} y={4} width={8} height={10} rx={1.5} fill="var(--color-bg-surface)" />
      {/* shoulder + body */}
      <path
      ...
      />
      {/* label */}
      <rect x={9} y={56} width={18} height={26} rx={1} fill={type === 'white' ? 'var(--color-bg-surface)' : 'var(--color-paper)'} opacity={0.92} />
      <rect x={12} y={62} width={12} height={1.5} fill={type === 'white' ? 'var(--color-gold)' : 'var(--color-wine-red)'} />
      <rect x={12} y={67} width={12} height={1} fill={type === 'white' ? 'var(--color-gold)' : 'var(--color-wine-red)'} opacity={0.6} />
    </svg>
  );
}
...
function AlertToggleCard({ label }: { label: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <Bell size={18} color="var(--color-gold)" strokeWidth={2} />
      </div>
...
export default function CellarSection() {
  ...
  return (
    <section
      style={{
        background: 'radial-gradient(ellipse 70% 50% at 50% 0%, rgba(201,168,76,0.06) 0%, transparent 58%), var(--color-bg-deep)',
          ...
          {wines.map((wine, i) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--color-text-muted)', marginBottom: 6, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                    <span>{cellar?.timelineFrom ?? 'From'}</span>
                    <span style={{ color: 'var(--color-gold)', fontWeight: 600 }}>{peakReadout}</span>
                ...
                {/* Meta lines */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 4, borderTop: '1px solid var(--color-border-soft)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--color-text-muted)' }}>
                    <span style={{ color: 'var(--color-text-muted)' }}>위치</span>
                    <span>{wine.place}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--color-text-muted)' }}>
                    <span style={{ color: 'var(--color-text-muted)' }}>취득</span>
                    <span>{wine.acquired}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--color-text-muted)' }}>
                    <span style={{ color: 'var(--color-text-muted)' }}>구매가</span>
                    <span style={{ color: 'var(--color-text-secondary)', fontWeight: 600 }}>{wine.price}</span>
                  </div>
                </div>
              </motion.div>
            );
